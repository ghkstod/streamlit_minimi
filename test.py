...
def main():
    ...
    if selected_bjdong_nm != None:
        ...
        with tab2:        
            ...
            '관심 키워드를 선택하세요.',
            filtered_data.columns.drop(['SGG_NM', 'BJDONG_NM'])
        )
            ...
            if len(options) > 0:
                ...
                selected_values = {}  # 선택된 옵션과 값들을 저장할 딕셔너리

                for option in options:
                    # 각 선택된 키워드에 대한 고유한 값을 가지는 selectbox 생성
                    selected_value = st.selectbox(
                        f'{option}의 값을 선택하세요.', 
                        ['선택하세요'] + list(filtered_data[option].unique()), 
                        key=option  # 각 selectbox에 고유한 키 제공
                    )
                    if selected_value != '선택하세요':
                        selected_values[option] = selected_value
                
                if len(selected_values) > 0:
                    # 모든 선택된 조건에 따라 데이터 필터링
                    filtered_rows = filtered_data.copy()
                    for option, value in selected_values.items():
                        filtered_rows = filtered_rows[filtered_rows[option] == value]

                    if not filtered_rows.empty:
                        st.write(filtered_rows)
                    else:
                        st.write('선택된 조건에 맞는 데이터가 없습니다.')
